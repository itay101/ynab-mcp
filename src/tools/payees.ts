...
export async function getPayee(args: { budget_id?: string; payee_id: string }, token?: string) {
  const client = getClient(token);
  const budgetId = args.budget_id ?? "last-used";
...
export async function listPayeeLocations(args: { budget_id?: string }, token?: string) {
  const client = getClient(token);
  const budgetId = args.budget_id ?? "last-used";
...
export async function getPayeeLocation(args: { budget_id?: string; payee_id: string }, token?: string) {
  const client = getClient(token);
  const budgetId = args.budget_id ?? "last-used";
